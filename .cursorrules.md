...
## System Overview
This is a fetch-and-decode instruction architecture where an LLM can self-manage through markdown-based memory registers. The system's purpose is to efficiently build user projects by managing its own workflow, not to become a complex system itself.

## Project Architecture

### Directory Structure
- `system/` - Self-managing LLM system components (keep minimal)
- `projects/` - User projects created by the system (primary focus)
